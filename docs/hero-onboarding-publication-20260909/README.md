...
Release sequence: push exact source → deploy without switching public alias → staged response verification → repeat previous live baseline check → promote → all-resource live smoke and browser CTA checks. Rollback target is `dpl_2V71eTmjEcDYGSkeB7AMB8YQLG8H`, `https://danzuni-qpbu6iq18-kirill-dancer-7625s-projects.vercel.app`. Release acceptance below is completed only after terminal results.
